...
7. Git: this studio lives on the orphan branch `claude/app-studio` of the
   ardaerturkk/ardavault repository (Arda's Obsidian vault is on `main`; never touch
   `main` or any other branch). Always work on and push to `claude/app-studio`. Never
...
3. At the end: STATE.md current, ARDA-INBOX.md current, delete STUDIO.LOCK, commit, push to claude/app-studio.
...
5. iOS verification: .github/workflows/ios-check.yml on a GitHub macos runner, triggered
   by pushing a tag ios-check-<app>-<n> (workflow_dispatch does not work here because this
   studio lives on a non-default branch of the ardavault repo). It runs
...
Arda must be able to do: git clone -b claude/app-studio --single-branch <ardavault repo> app-studio (or git pull), flutter pub get, open ios/Runner.xcworkspace, pick his
