...
if [ "$elapsed" -lt "$RUN_MINUTES" ]; then
  echo "Studio run: ${elapsed}/${RUN_MINUTES} min used. Read STATE.md, do the next pipeline step from CLAUDE.md, verify it, update STATE.md, commit and push to claude/app-studio. If everything is blocked on Arda, create .studio/WAITING_ON_ARDA, explain in ARDA-INBOX.md, push, and end." >&2
  exit 2
fi
...
if [ -f "$dir/STUDIO.LOCK" ] && [ "$elapsed" -lt $((RUN_MINUTES + 15)) ]; then
  echo "Run budget reached. Update STATE.md, delete STUDIO.LOCK, commit and push to claude/app-studio, then end." >&2
  exit 2
fi
exit 0
