...
abstract class BookStorage {
  ...
  /// Moves an unreadable saved file aside so the next save cannot overwrite
...
class FileStorage implements BookStorage {
  ...
  Future<String?> read() async {
    final f = await _file('sheetwise.json');
    return await f.exists() ? f.readAsString() : null;
  }

  @override
  Future<void> write(String json) async {
    final tmp = await _file('sheetwise.json.tmp');
    await tmp.writeAsString(json, flush: true);
    await tmp.rename((await _file('sheetwise.json')).path);
  }
  ...
  Future<void> setAsideUnreadable() async {
    ...
    await f.rename((await _file('sheetwise-unreadable-$stamp.json')).path);
  }
}
