...
/// Where the book's JSON lives. Tests use [MemoryStorage].
abstract class BookStorage {
  Future<String?> read();
  Future<void> write(String json);

  /// Moves an unreadable saved book aside so the next save cannot overwrite
...
class FileStorage implements BookStorage {
  ...
  Future<String?> read() async {
    final f = await _file('bayramrounds.json');
    return await f.exists() ? f.readAsString() : null;
  }

  @override
  Future<void> write(String json) async {
    final tmp = await _file('bayramrounds.json.tmp');
    await tmp.writeAsString(json, flush: true);
    await tmp.rename((await _file('bayramrounds.json')).path);
  }
  ...
  Future<void> setAsideUnreadable() async {
    ...
    await f.rename((await _file('bayramrounds-unreadable-$stamp.json')).path);
  }
}
